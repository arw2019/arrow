...
struct IndexState {
  ...
  using ThisType = IndexState<ArrowType>;
...
struct IndexImpl : public ScalarAggregator {
  using ThisType = IndexImpl<ArrowType>;
  using ArrayType = typename TypeTraits<ArrowType>::ArrayType;

  explicit IndexImpl(const std::shared_ptr<DataType>& out_type,
                     const FindIndexOptions& options)
  ...
  IndexState<ArrowType> state;
  FindIndexOptions options;
};

struct IndexInitState {
  ...
  IndexInitState(KernelContext* ctx, const DataType& in_type,
                 const std::shared_ptr<DataType>& out_type, const FindIndexOptions& options)
  ...
  enable_if_t<is_number_type<Type>::value, Status> Visit(const Type&) {
    state.reset(new IndexImpl<Type>(out_type, options));
    return Status::OK();
  }
...
std::unique_ptr<KernelState> IndexInit(KernelContext* ctx, const KernelInitArgs& args) {
  IndexInitState visitor(ctx, *args.inputs[0].type,
                         args.kernel->signature->out_type().type(),
                         static_cast<const FindIndexOptions&>(*args.options));
